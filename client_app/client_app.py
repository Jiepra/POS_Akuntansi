...
class ModernPOS(QMainWindow):
    ...
    def setup_ui(self):
        ...
        """)
        self.cart_tree.setColumnWidth(0, 200) # Sesuaikan lebar kolom
        self.cart_tree.setColumnWidth(1, 120) # Sesuaikan lebar kolom untuk tombol qty
        self.cart_tree.setColumnWidth(2, 120)
        self.cart_tree.setColumnWidth(3, 120)
        self.cart_tree.setColumnWidth(4, 60) # Lebar untuk tombol aksi
    ...
    def update_cart_display(self):
        ...
        # Nonaktifkan sinyal sementara untuk menghindari refresh berulang
        ...
        for code, item in self.cart.items():
            ...
            # Tambahkan tombol untuk mengubah kuantitas dan menghapus
            btn_minus = QPushButton("-")
            btn_minus.setFixedSize(25, 25)
            btn_minus.setStyleSheet("font-weight: bold; background-color: #f44336; color: white; border-radius: 5px;")
            btn_minus.clicked.connect(lambda _, c=code: self.change_cart_qty(c, -1)) # Lambda untuk passing argumen

            btn_plus = QPushButton("+")
            btn_plus.setFixedSize(25, 25)
            btn_plus.setStyleSheet("font-weight: bold; background-color: #4CAF50; color: white; border-radius: 5px;")
            btn_plus.clicked.connect(lambda _, c=code: self.change_cart_qty(c, 1))

            btn_remove = QPushButton("X")
            btn_remove.setFixedSize(25, 25)
            btn_remove.setStyleSheet("font-weight: bold; background-color: #d32f2f; color: white; border-radius: 5px;")
            btn_remove.clicked.connect(lambda _, c=code: self.remove_from_cart(c))

            # Buat widget container untuk tombol agar bisa diletakkan di QTreeWidget
            qty_buttons_widget = QWidget()
            qty_buttons_layout = QHBoxLayout(qty_buttons_widget)
            qty_buttons_layout.setContentsMargins(0, 0, 0, 0)
            qty_buttons_layout.setSpacing(5)
            qty_buttons_layout.addWidget(btn_minus)
            qty_buttons_layout.addWidget(QLabel(str(item['qty']))) # Tampilkan Qty di tengah tombol
            qty_buttons_layout.addWidget(btn_plus)
            
            remove_button_widget = QWidget()
            remove_button_layout = QHBoxLayout(remove_button_widget)
            remove_button_layout.setContentsMargins(0, 0, 0, 0)
            remove_button_layout.setSpacing(0)
            remove_button_layout.addStretch()
            remove_button_layout.addWidget(btn_remove)
            remove_button_layout.addStretch()

            self.cart_tree.setItemWidget(tree_item, 1, qty_buttons_widget) # Kolom Qty
            self.cart_tree.setItemWidget(tree_item, 4, remove_button_widget) # Perlu menambahkan kolom 'Aksi'
    ...
    def change_cart_qty(self, product_code, change):
        if product_code in self.cart:
            conn = connect_db()
            cursor = conn.cursor()
            cursor.execute("SELECT stok FROM barang WHERE kode = %s", (product_code,))
            current_stock_db = cursor.fetchone()[0]
            conn.close()

            new_qty = self.cart[product_code]['qty'] + change
            
            if new_qty > 0 and new_qty <= current_stock_db:
                self.cart[product_code]['qty'] = new_qty
            elif new_qty <= 0:
                self.remove_from_cart(product_code)
                return # Langsung kembali setelah menghapus
            elif new_qty > current_stock_db:
                QMessageBox.warning(self, "Stok Tidak Cukup", f"Stok {self.cart[product_code]['name']} tersisa: {current_stock_db}")
    ...
    def finalize_payment(self, method):
        try:
            ...
            # Mendapatkan metode pembayaran untuk disimpan
            payment_method_str = "QRIS" if method == "qris" else "Kartu"
            
            # Iterasi setiap item dalam keranjang untuk dicatat ke tabel penjualan
            for code, item in self.cart.items():
                cursor.execute("UPDATE barang SET stok = stok - %s WHERE kode = %s",
                                 (item['qty'], code))
                cursor.execute("""
                    INSERT INTO penjualan (
                        id_pesanan,        -- Kolom baru yang perlu diisi
                        tanggal, 
                        pelanggan, 
                        kode_barang, 
                        jumlah, 
                ...
                """, (
                    self.current_order_number, # <-- Masukkan nilai id_pesanan di sini
                    ...
                    item['price'],
                    item['price'] * item['qty'],
                    payment_method_str # Masukkan metode pembayaran
                ))
                
            conn.commit()
            conn.close() # Penting: tutup koneksi setelah commit

            # Generate receipt
            self.generate_receipt(method)
            
            # Record transaction (ke Excel)
            # Pastikan record_transaction juga diperbarui untuk kolom metode_pembayaran
            ...
            self.clear_cart()
            self.load_products() # Refresh tampilan produk setelah stok berubah
            self.current_order_number = self.generate_order_number() # Generate nomor pesanan baru
            
        except mysql.connector.Error as err:
            QMessageBox.critical(self, "Database Error", f"Error database: {err}")
            # print(f"Database Error: {err}") # Untuk debugging
            if 'conn' in locals() and conn.is_connected():
                conn.rollback() # Lakukan rollback jika ada error database
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Error umum: {str(e)}")
            # print(f"General Error: {e}") # Untuk debugging
        finally:
            if 'conn' in locals() and conn.is_connected():
                conn.close() # Pastikan koneksi selalu tertutup
